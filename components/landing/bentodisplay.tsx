...
export function BentoGridDisplay() {
  return (
    <BentoGrid className="w-full px-4 md:auto-rows-[20rem] pb-10">
      {items.map((item, i) => (
        <BentoGridItem
          key={i}
          title={item.title}
...
const SkeletonTwo = () => {
  ...
  return (
    ...
    >
      {arr.map((_, i) => (
        <motion.div
          key={"skelenton-two" + i.toString()}
          variants={variants}
          style={{
            maxWidth: `${(Math.random() * (100 - 40) + 40).toString()}%`,
          }}
  ...
  return (
    ...
    >
      ...
      >
        <Image src="/globe.svg" alt="avatar" height="100" width="100" className="rounded-full h-10 w-10" />
        <p className="sm:text-sm text-xs text-center font-semibold text-neutral-500 mt-4">
          Practice your data structures and algorithms
        </p>
